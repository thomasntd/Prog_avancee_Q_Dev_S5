...
for dataFileName in dataFileNames:
    ...
    allSpeedups: list[float] = list()
    for key in sorted(list(durations.keys())):
        for elt in durations[key]:
            allSpeedups.append(durations[1][5]/elt)

    plt.figure(figsize=(13, 10))
    plt.scatter([[i]*len(durations[i]) for i in sorted(durations.keys())], allSpeedups, color='grey', label='Tous les speedup')
    plt.plot(range(1, len(speedups)+1), speedups, marker='o', linestyle='-', color='g', label='Speedup médian')
    plt.plot(range(1, len(speedups)+1), range(1, len(speedups)+1), linestyle='--', color='r', label='Target Speedup')

    plt.xlabel("Nombre de processus")
    plt.ylabel("Speedup ( T1/Tp )")
    plt.title(f"Évolution du Speedup du programme {prog_name[dataFileName.split("_")[0]]} en fonction du nombre de processus \n"
    ...
    plt.savefig("graph/simple_nuage/graph" + dataFileName + ".png", dpi=300)
    plt.close()
