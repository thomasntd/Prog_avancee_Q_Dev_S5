...
for dataFileNames in tabDataFileNames:
    ...
    for speedups in allSpeedUps:
        plt.plot(range(1, len(speedups)+1), speedups, marker='o', linestyle='-', color=colors[i], label=f'Programme {prog_name[dataFileNames[i].split("_")[0]]}')
        i += 1
    ...
    plt.title(
        f"Comparaison de l'évolution du Speedup médian de différents programmes en fonction du nombre de processus\n"
        f"pour une charge de {dataFileName.split("_")[1][3:]} points travail sur un test de scalabilité forte")
    plt.legend()
    plt.grid(True)
    plt.xticks(range(1, len(allSpeedUps[0]) + 1))
    plt.yticks(range(1, len(allSpeedUps[0]) + 1))

    plt.savefig("graph/compare/graph" + dataFileNames[1].split("_")[1] + ".png", dpi=300)
    plt.close()
...
for dataFileNames in tabDataFileNames:
    ...
    plt.title(
        f"Comparaison de l'évolution du Speedup médian de différents programmes en fonction du nombre de processus\n"
        f"pour une charge de {dataFileName.split("_")[1][3:]} points travail sur un test de scalabilité faible")
    plt.legend()
    plt.grid(True)
    plt.xticks(range(1, len(allSpeedUps[0]) + 1))
